    ...
    function _renderForm(context) {
      try {
        ...
        if (stItemInternalId) {
          ...
          var objPlans = api.getOffers(stItemInternalId, objExtendConfig);
  
          log.debug("OFFER MODAL SUITELET: Offers JSON Response", objPlans);
          objPlans = JSON.parse(objPlans.body);
          var arrPlans = objPlans.plans;
